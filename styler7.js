(function() {
  ...
  // Logo URLs — SVG primary, PNG fallback
  const LOGO_SVG = 'https://raw.githubusercontent.com/RedRavenLabs/OGLabsCannes/da01990d1f8039ff961fbf1d80b6d9a5fb53ffe7/0G-Logo-Purple_Hero.svg';
  const LOGO_PNG = 'https://github.com/RedRavenLabs/OGLabsCannes/blob/main/0G-Logo-Purple_Hero.png?raw=true';

  window.init0GBranding = function(theme) {
    const logos = document.querySelectorAll('img.og-logo');

    logos.forEach(logo => {
      // If the logo already loaded successfully, skip it
      if (logo.complete && logo.naturalWidth > 0) return;

      // Try SVG first, fall back to PNG on error
      logo.onerror = function() {
        // Only fall back once — avoid infinite loop
        if (logo.src !== LOGO_PNG) {
          logo.onerror = null;
          logo.src = LOGO_PNG;
        }
      };
